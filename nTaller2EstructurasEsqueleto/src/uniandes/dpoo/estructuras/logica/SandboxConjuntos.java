...
public class SandboxConjuntos
{
    ...
    {
    	String primero = null; //Utilizamos el metodo first() para obtener al primer elemento
    	if(arbolCadenas.isEmpty() == false) { //si no esta vacio
    		primero = arbolCadenas.first(); 
    	}
        return primero;
    }
    ...
    {
    	String ultimo = null; //Tal como usamos first, usamos last
    	if(arbolCadenas.isEmpty() == false) {
    		ultimo = arbolCadenas.last();
    	}
        return ultimo;
    }
    ...
    {
    	//Paso 1: Creamos un nuevo arbol
    	NavigableSet<String> nuevo_arbol = new TreeSet<String>();
    	
    	//Paso 2: Recorremos todo el arbol antiguo
    	for(String elemento: arbolCadenas) {
    		nuevo_arbol.add(elemento.toUpperCase());
    	}
    	//Paso 3: Lo hacemos efectivo
    	arbolCadenas = nuevo_arbol;
    }
    ...
    public TreeSet<String> invertirCadenas( )
    {
        ...
        TreeSet<String> nuevo_arbol = new TreeSet<>(Comparator.reverseOrder()); //Ahora este TreeSet sigue un orden inverso
        nuevo_arbol.addAll(arbolCadenas); //Entonces cuando agregamos elementos, ya no se ordenan en orden lexicografico sino al reves
        return nuevo_arbol;
    }
    ...
    {
    	int i = 0;
    	boolean hace_parte_elementos_del_conjunto = true;
    	int tamanio = otroArreglo.length;
    	while(i<tamanio && hace_parte_elementos_del_conjunto == true) {
    		String elemento = otroArreglo[i];
    		if(arbolCadenas.contains(elemento) == false) {
    			hace_parte_elementos_del_conjunto = false;
    		}
    		i++;
    	}
        return hace_parte_elementos_del_conjunto;
    }
