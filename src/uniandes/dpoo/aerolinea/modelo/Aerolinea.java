...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	Vuelo vuelo_encontrado = null;
    	if (!ruta.equals(null)) {
    	for(Vuelo vuelo: getVuelos()) {
    		if ((vuelo.getRuta() == ruta) && (vuelo.getFecha() == fechaVuelo)){
    ...
    {
    	Collection<Tiquete> listado_tics = new ArrayList<Tiquete>();
        // TODO implementar
    	for(Vuelo v: getVuelos()) {
    		listado_tics.addAll(v.getTiquetes());
    	}
        return listado_tics;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	//Encontrar al avion
    	Avion avi = new Avion(nombreAvion, 0);
    	for(Avion av: getAviones()) {
    		if (av.getNombre().equals(nombreAvion)) {
    			avi = av;
    			break;
    		}
    	}
    	if (avi.getCapacidad() == 0) {
    		throw new Exception();
    	}
    	//Saber si no esta ocupado
    	//Avion no debe estar ocupado
    	Vuelo vev = getVuelo(codigoRuta, fecha);
    	if (vev.getAvion() != avi) {
    		Vuelo vuelo = new Vuelo(getRuta(codigoRuta), fecha,avi);
    		vuelos.add(vuelo);
    	}else {
    		throw new Exception();
    	}
    ...
    {
        // TODO Implementar el método
    	Cliente c1 = clientes.get(identificadorCliente);
    	//Valor antes
    	int valor_antes = c1.calcularValorTotalTiquetes();
    	Vuelo vev = getVuelo(codigoRuta, fecha);
    	String temporada_baja = "01,02,03,04,05,09,10,11";
    	CalculadoraTarifas calc = new CalculadoraTarifasTemporadaAlta();
    	if (temporada_baja.contains(fecha.substring(5, 7))) {
        	calc = new CalculadoraTarifasTemporadaBaja();
    	}
    	vev.venderTiquetes(c1, calc, cantidad);
    	int valor_despues = c1.calcularValorTotalTiquetes();
    	
        return valor_despues - valor_antes;
    }
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
        // TODO Implementar el método
    	Vuelo vev = getVuelo(codigoRuta, fecha);
    	//Entiendo que registrar es que los tiquetes se hayan usado
    	for(Tiquete t1 : vev.getTiquetes()) {
    		t1.marcarComoUsado();
    	}    	
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	...
    	Cliente c1 = clientes.get(identificadorCliente);
    	//retornar el metodo
    	int precio = c1.calcularValorTotalTiquetes();
